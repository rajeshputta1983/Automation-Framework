 ...
 * @author Rajesh Putta
 */
public class Task8OutputPlugin extends OutputPlugin{
	public TaskStatus process(Map<String, String> configuration, IContext context, TaskStatusManager taskStatusManager) {
		System.out.println("inside Task 8 output plugin ...");
		
		TaskStatusContext taskStatusContext=taskStatusManager.getOutputTasksContext();
		
		TaskStatus status=taskStatusContext.get("task6");
		
		System.out.println("Task 6 Result ::"+status.getResult());
